...
const GlyStockTransaction = require('../models/GlyStockTransaction');
...
exports.create = async (req, res) => {
  try {
    ...
    const stockTx = new GlyStockTransaction({ type, productId, productName, quantity, unitPrice, total, notes });
    await stockTx.save();
...
exports.list = async (req, res) => {
  try {
    ...
    const records = await GlyStockTransaction.find(filter).sort({ date: -1 });
    res.json(records);
  } catch (err) {
    console.error(err);
